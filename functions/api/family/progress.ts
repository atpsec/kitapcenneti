...
export const onRequestGet = async (context: AuthContext) => {
  try {
    ...
    if (!/^[A-Za-z0-9_-]{1,64}$/.test(childId)) return authResponse({ error: 'Ungültiges Profil' }, 400, context)
    if (!context.env.DB) return authResponse({ error: 'Fortschrittsservice ist noch nicht eingerichtet', code: 'configuration_missing' }, 503, context)
    ...
    if (!child) return authResponse({ error: 'Profil nicht gefunden' }, 404, context)
  ...
  } catch (error) { console.error('Progress read error', error); return authResponse({ error: 'Fortschritt konnte nicht geladen werden' }, 500, context) }
}
...
export const onRequestPost = async (context: AuthContext) => {
  ...
  } catch (error) { console.error('Progress write error', error); return authResponse({ error: 'Fortschritt konnte nicht gespeichert werden' }, 500, context) }
}
