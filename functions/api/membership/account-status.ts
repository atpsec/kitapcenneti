...
export const onRequestGet = async (context: AuthContext) => {
  try {
    ...
    const membership = await context.env.DB.prepare(
      'SELECT status, updated_at as updatedAt, current_period_end as currentPeriodEnd, customer_id as customerId, subscription_id as subscriptionId, cancel_at_period_end as cancelAtPeriodEnd FROM memberships WHERE account_id = ? OR (account_id IS NULL AND lower(email) = lower(?)) ORDER BY updated_at DESC LIMIT 1',
    ).bind(account.id, account.email).first?.<{ status?: string; updatedAt?: string; currentPeriodEnd?: string; customerId?: string; subscriptionId?: string; cancelAtPeriodEnd?: number }>()
    const active = membership?.status === 'active' || membership?.status === 'trialing'
    return authResponse({ plan: active ? 'family_plus' : 'free', status: membership?.status || 'inactive', updatedAt: membership?.updatedAt, currentPeriodEnd: membership?.currentPeriodEnd, customerId: membership?.customerId, subscriptionId: membership?.subscriptionId, cancelAtPeriodEnd: membership?.cancelAtPeriodEnd === 1 }, 200, context)
  } catch (error) {
