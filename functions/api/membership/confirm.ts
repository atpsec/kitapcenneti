...
export const onRequestPost = async (context: AuthContext) => {
  try {
    ...
    if (!context.env.DB) return authResponse({ error: 'Üyelik servisi henüz yapılandırılmadı', code: 'configuration_missing' }, 503, context)
    const body = (await context.request.json()) as { sessionId?: unknown }
    const sessionId = safeId(body.sessionId, 'cs_')
    if (!sessionId) return authResponse({ error: 'Geçersiz ödeme oturumu' }, 400, context)
    ...
    if (!response.ok || !session.subscription?.id) return authResponse({ error: 'Üyelik doğrulanamadı' }, 502, context)
    const linkedAccountId = session.metadata?.account_id || session.client_reference_id || ''
    if (linkedAccountId !== account.id) return authResponse({ error: 'Ödeme hesabıyla giriş hesabı eşleşmiyor' }, 403, context)
    const activeStatuses = new Set(['active', 'trialing'])
    if (!activeStatuses.has(session.subscription.status || '')) return authResponse({ error: 'Üyelik henüz aktif değil' }, 402, context)
    ...
    if (error instanceof Error && error.message === 'DB_NOT_CONFIGURED') return authResponse({ error: 'Üyelik servisi henüz yapılandırılmadı', code: 'configuration_missing' }, 503, context)
    return authResponse({ error: 'Üyelik doğrulanamadı' }, 500, context)
  }
}
